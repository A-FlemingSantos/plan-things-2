...
describe('KanbanBoard file sync without board reload', () => {
  ...
  it('unshares a plan file locally, removes matching attachments, and skips board reload', async () => {
    boardState.columns = [
      {
        id: 'col-1',
        title: 'Backlog',
    ...
    apiMock.apiRequest.mockImplementation((path, options = {}) => {
      if (path === '/api/files/plans/plan-1') {
      ...
      if (path === '/api/files/file-2/share/plans/plan-1' && options.method === 'DELETE') {
    ...
    await userEvent.click(screen.getByRole('button', { name: /^Arquivos$/i }))
